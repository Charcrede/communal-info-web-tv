...
export default function PubliReportagePage() {
  ...
  const fetchArticles = async (pageNum: number = 1) => {
    ...
    const response = await axios.get(`${apiUrl}/articles/?rubric=4`).then((resp) => {
      setLoading(false)
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-900 dark:to-gray-800 transition-colors duration-300">
      <div className="max-w-7xl mx-auto py-10 px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <Sidebar currentCategory="publi-reportage" toExclude={4}/>
        </div>
      </div>
    </div>
  );
